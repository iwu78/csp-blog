...
The Creative Commons Zero license is a commercial copyright which means that other people cannot use the code for their own use and must credit when linking to the code. Open Source MIT and GPL are both open source licenses that allow other people to use the code and modify it for their own use, but they differ in that GPL license is a strong copyleft license and sets restrictive obligations on use on a personal server, whereas MIT is a permissive open source license and does not set these obligations.
...
Qualcomm and Linux's use of open source licenses like GPL licenses allow users to pay to use their code, or for a license to use it, and then they can profit from it, but have to pay that fee. This benefits the buyer as they can now use Qualcomm's code, and also Qualcomm as they generate revenue from this.
...
Businesses that rely on Open Source software may still need to generate income to pay for code, and to maintain and modify the code they need for their own purposes. Depending on the Open Source license, businesses may have to pay a commission to the original owner of the code, or they may be able to use and profit from it directly, depending on the open source license.
